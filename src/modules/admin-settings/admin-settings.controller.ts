...
export class AdminSettingsController {
  ...
  async getAdminSettingsByAdminId(@Param('adminId') adminId: string) {
    const settings = await this.adminSettingsService.getAdminSettingsByAdminId(Number(adminId));
    if (!settings) {
      throw new NotFoundException(`Admin settings for Admin ID ${adminId} not found`);
    }
    return settings;
  }
  ...
  async createAdminSettings(@Body() createAdminSettingsDto: UpdateAdminSettingsDto) {
    ...
    const admin = await this.adminSettingsService.getAdminByAdminId(createAdminSettingsDto.adminId);
    if (!admin) {
      throw new BadRequestException(`Admin with ID ${createAdminSettingsDto.adminId} does not exist`);
    }

    // Check if settings already exist for this admin
    const existingSettings = await this.adminSettingsService.getAdminSettingsByAdminId(createAdminSettingsDto.adminId);

    // If settings already exist, throw an exception, otherwise proceed with creation
    if (existingSettings) {
      throw new BadRequestException('Admin settings for this admin already exist.');
    }
  ...
  async updateAdminSettings(
    ...
    return this.adminSettingsService.updateAdminSettings(Number(adminId), updateAdminSettingsDto);
  }
}
