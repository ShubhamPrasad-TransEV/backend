...
export class CartService {
  ...
  async addToCart(addToCartDto: AddToCart, quantity: number) {
    ...
    if (existingItem) {
        ...
        console.log(`Item does not exist. New total quantity will be ${newQuantity}.`);
    }

    // Debugging output
    console.log(`Adding ${quantityInt} to cart.`);
    console.log(`Existing quantity in cart: ${existingItem ? existingItem.quantity : 0}`);
    ...
    await this.prisma.product.update({
        where: { id: productId },
        data: { quantity: product.quantity - Math.min(quantityInt, product.quantity) }, // Deduct only what can be added
    });
    ...
    await this.prisma.quantitymanagement.create({
        data: {
            ...
            afterupdatequantity: Math.max(product.quantity - Math.min(quantityInt, product.quantity), 0), // Log what it will be after deduction
        },
    });
...
async clearCart(userId: number) {
    ...
    const cartItems = await this.prisma.cart.findMany({ where: { userId: userIdInt } });
    ...
    for (const item of cartItems) {
        ...
        if (!product) {
            throw new NotFoundException(`Product not found for ID: ${item.productId}`);
        }
